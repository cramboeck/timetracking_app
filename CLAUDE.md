...
| ~~A6~~ ✅ | **GPS-Stempelung (Arbeitszeit)** | Basis ✅ (97847e0) + Phase 2 ✅ (1b71956, 7.9.) | Basis: Migration 017, Org-Toggle (Team Management, Default aus), Server verwirft Koordinaten ohne Freigabe, One-Shot-Geolocation mit 6s-Timeout in der AttendanceBar (Stempeln hängt nie an GPS), OSM-Pins in Team- und Mein-Bereich-Ansicht. **Phase 2 (1b71956):** Zuordnung „gestempelt bei Kunde X" — Migration 018 (`customers.latitude/longitude/geocoded_at`, `work_sessions.clock_in/out_customer_id`); `geocodingService` (Nominatim/OSM, DACH-countrycodes, 8s-Timeout, best effort — `geocoded_at` wird auch bei Fehlschlag gesetzt, Adressänderung resettet); Geocoding beim Kunden-Speichern (fire-and-forget) + `geocodeCustomersCron` (Boot+2min für Bestandskunden, täglich 03:30 Nachzügler, 1,1s Nominatim-Abstand); Match im Stempel-Moment via Haversine (Radius 300 m, kein Match bei Accuracy >500 m — lieber kein Name als ein falscher), Kundenname am OSM-Pin in Mein Bereich + Team-Ansicht. ⚠️ Nominatim war in der Test-Sandbox proxy-geblockt — Geocoding-Zweig auf Prod per Server-Log „📍 Kunde … geocodiert" verifizieren. Design fix bleibt: Org-Schalter standardmäßig aus (DSGVO/Mitbestimmung), Stempeln funktioniert IMMER auch ohne GPS. ⚠️ Geofencing/Auto-Stempeln ist in der PWA nicht zuverlässig machbar — bräuchte native App (Capacitor, eigenes Projekt). |
...
- Offline-Sync deckt Zeiteinträge + Ticket-Kommentare ab (9.9.); andere Aktionen (z.B. Task-Updates) weiterhin nicht.
...
| ✅ | **Offline-Sync für Ticket-Kommentare** (erledigt 9.9.) | `offlineStorage.ts` + `useOfflineEntrySync.ts` + `TicketDetail.tsx` + `tickets/03-comments-attachments.ts` | erledigt | Kommentare landen bei Netzabriss/5xx in einer localStorage-Queue (`pending_ticket_comments`) und werden vom bestehenden Sync-Hook mitverarbeitet (online-Transition, 30s-Retry, `offline-queue-changed`-Event für sofortigen Versuch). Idempotenz: `clientId` wird serverseitig als Kommentar-ID verwendet — Retry liefert Bestehendes zurück (200) OHNE erneute Benachrichtigungen. OfflineBanner zählt Kommentare mit und listet fehlgeschlagene (Retry/Verwerfen). ⚠️ Lehre: TanStack v5 `networkMode: 'online'` PAUSIERT Mutationen bei offline (Promise hängt, Reload = Datenverlust) — die Kommentar-Mutation läuft mit `networkMode: 'always'`, damit der Fehler sofort in die Queue fällt. **Task-Updates bewusst nicht** (Update-Konflikte, geringer Feldwert — bei Bedarf nachrüsten). |
...
| ✅ | **Portal: VK-Preiskonzept für Lizenzansicht** (erledigt 7.9.) | Migration 019 + `sevdesk.ts` + `LineItemReview.tsx` + `customer-portal.ts` | erledigt | `invoice_line_items.resell_price` (VK **pro Einheit**, fester Preis — kein Prozentsatz) + `customer_product_prices` als Preis-Gedächtnis pro Kunde+Produkt (product_key = SKU, sonst lower(description)): einmal im LineItemReview gepflegt (`PATCH /line-items/:id/resell-price`), erben künftige Positionen den Preis automatisch via COALESCE beim Lesen — kein monatliches Nachpflegen. Portal-Lizenzansicht reaktiviert (403 + canViewLicenses-Override raus): alle Beträge ausschließlich aus VK, „Monatliche Kosten" = jüngster Belegmonat statt Alltime-Summe, Position ohne VK → „—". CRM-Lizenzen-Tab zeigt VK-Summe + amber „VK fehlt (N)"-Chip. Verifiziert: EK-Leak-Check der Portal-Response sauber, 403 ohne Berechtigung, Gedächtnis-Upsert/-Löschung, Zwei-Monats-Aggregation. |
| ✅ | **Portal: Lizenz-Self-Service** (erledigt 9.9.) | Migration 020 + `customer-portal.ts` + `sevdesk.ts` + `PortalLicenses.tsx` + `CustomerLicenses.tsx` | erledigt | Tabelle `license_requests` (Typ increase/decrease/new/cancel, Zustandsmaschine pending → approved/rejected → completed — completed nur aus approved, sonst 400). Portal: „Anfrage stellen"-Button + „Änderung anfragen" pro Produkt (Formular vorbefüllt), „Ihre Anfragen"-Liste mit Status-Badges + Admin-Rückmeldung; Neuprodukt-Anfragen gehen auch ohne zugeordnete Positionen; Spam-Bremse max. 20 offene/Kunde. Admin: Sektion „Lizenz-Anfragen aus dem Portal" im CRM-Lizenzen-Tab (pending zuerst, Genehmigen direkt, Ablehnen mit optionaler Begründung, „Als erledigt markieren" nach Provisionierung) — Entscheidungen nur Org-Admin/Owner (requireBillingFeature ist Paket, keine Berechtigung). Benachrichtigungen: Org-Admins per Mail+Push bei neuer Anfrage, Anfragender per Mail bei Genehmigung/Ablehnung (best effort). Provisionierung bleibt manuell beim Distributor (Automatik = eigener Roadmap-Punkt). |
...
| ✅ | **Portal: Lizenz-Self-Service** | Erledigt 9.9. — siehe Sprint-J-Tabelle (license_requests, Genehmigungsworkflow, Mail/Push) |
