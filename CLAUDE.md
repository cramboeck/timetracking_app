...
| ⬜ | **Admin-Zeitenübersicht (alle Teammitglieder)** | `src/components/AdminTeamTimeView.tsx` (neu), `server/src/routes/entries.ts` | 3-4h | Nur für Rollen `admin` und `manager`. Dashboard mit Dropdown zur Mitarbeiterauswahl (oder „Alle"), Zeiten-Liste analog zur eigenen Ansicht aber org-weit, Filter nach Zeitraum/Buchungsart/Mitarbeiter. CSV-Export für gesamte Organisation. Backend: neuer Endpoint `GET /api/entries/team` mit `organization_id`-Filter. Erreichbar unter „Berichte" → „Team-Zeiten". |

---

*Zuletzt aktualisiert: 12.6.2026 — Sprints 1–3 + Sprint A + Sprint B ✅ komplett. Interne Arbeitszeit-Erfassung mit Buchungsarten (Projekt/Intern/Abwesenheit), Badges, Filter und Auswertungs-Tab inkl. Export für Steuerberater/Lohnabrechnung. Sprint F erweitert: Admin-Zeitenübersicht für Teammitglieder. Verbleibend: Sprint C-F (Portal-Features).*
