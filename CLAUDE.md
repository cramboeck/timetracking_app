...
**Pass 2 ✅ (2c28273):** Zeiterfassung migriert — ManualEntryModern + Stopwatch (Selects/Textareas/Labels), Edit-Dialog (lokale `inputClasses` = Kit-Look, Karten-Layout unverändert) + Massenbearbeitung. Bewusst nicht: kompakte Filterleiste, Spezial-Widgets (SearchableSelect/DatePicker/Dauer-Feld). **Offen (weitere Pässe):** CustomerHub-Dialoge (Kunde/Projekt/Kontakt), Leads/InteractionForm, Settings-Formulare. Muster: Feld für Feld auf `Input/Select/Textarea` mit `label`-Prop, Grids auf `FormRow`.
