...
| ~~A6~~ 🟡 | **GPS-Stempelung (Arbeitszeit)** | Basis ✅ (97847e0, 7.9.) | Basis umgesetzt: Migration 017, Org-Toggle (Team Management, Default aus), Server verwirft Koordinaten ohne Freigabe, One-Shot-Geolocation mit 6s-Timeout in der AttendanceBar (Stempeln hängt nie an GPS), OSM-Pins in Team- und Mein-Bereich-Ansicht. **Offen: Phase 2** (Zuordnung „gestempelt bei Kunde X“ per Distanz). Ursprüngliches Design: Browser-Geolocation beim Ein-/Ausstempeln — Position NUR im Stempel-Moment, kein Tracking. Design fix: Org-Schalter **standardmäßig aus** (DSGVO/Mitbestimmung, Zweckbindung Arbeitszeitnachweis), Stempeln funktioniert IMMER auch ohne GPS (Fallback bei verweigerter Berechtigung), `work_sessions` + Koordinaten/Genauigkeit (guarded Migration), Karten-Link (OpenStreetMap) in Berichte → Arbeitszeit und „Mein Bereich" (Mitarbeiter sieht eigene Positionen — Transparenz). Phase 2 (später): Zuordnung „gestempelt bei Kunde X" per Distanz zu Kundenadressen. ⚠️ Geofencing/Auto-Stempeln ist in der PWA nicht zuverlässig machbar — bräuchte native App (Capacitor, eigenes Projekt). |
